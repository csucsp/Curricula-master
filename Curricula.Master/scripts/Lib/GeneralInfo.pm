...
{
	my ($lang) = (@_);
	my $output_file = Common::GetExpandedTemplateWithLang("out-tables-foreach-semester-file", $lang);
    my ($begin_tag, $end_tag) = ("<<", ">>");
	my $output_txt = "";
	my $total_credits = 0;
	my $this_line   = $Common::config{course_fields};
	# TODO All KeyLogs are duplicated
 	#Util::print_message("XYZ course_field=$this_line");
	#Common::print_keyLog("course_fields");
	#exit;
...
#			$this_course_info{CourseName} .= " ($Common::config{dictionary}{Pag}~\\pageref{sec:$codcour})~$pdflink";
		...
		if($cred_left > 0 && $Common::config{institution} eq "SPC")
		{
...
# 	print Dumper \%{$Common::counts{map_cred_area}{9}};
	...
	Util::write_file($output_file, $output_txt);
        ...
        my $ncredits_file = Common::GetTemplate("out-ncredits-file");
	Util::write_file(Common::GetTemplate("out-ncredits-file"), "$Common::config{ncredits}\\xspace");
...
sub generate_laboratories()
{
	...
	for(my $semester=1; $semester <= $Common::config{n_semesters} ; $semester++)
	{
		foreach my $codcour ( @{$Common::courses_by_semester{$semester}} )
		{
			...
			{
				my $course_name = $Common::course_info{$codcour}{$Common::config{language_without_accents}}{course_name};
				if($Common::course_info{$codcour}{labtype} eq "")
				{	Util::print_message("Course ".Util::red($codcour)." - ".Util::red($course_name)." (Sem #$semester) has not LabType ... did you forget?");
				...
				my $this_course = "\\section*{$codcour. $course_name ($Common::config{dictionary}{$Common::course_info{$codcour}{course_type}}) ";
	...
	Util::write_file(Common::GetTemplate("out-laboratories-by-course-file"), $output_txt);
...
{
	my $output_file = Common::GetTemplate("out-distribution-of-credits-by-area-by-semester-file");
	my $output_txt 	= "";
...
{
	my $LU_file = Common::GetTemplate("in-LU-file");
	if(not -e $LU_file)
	{	return;		}
...
{
	my ($type, $lang) = (@_);
	my ($in_file, $out_file) = (Common::GetExpandedTemplateWithLang("in-description-foreach-$type-file" , $lang), 
								Common::GetExpandedTemplateWithLang("out-description-foreach-$type-file", $lang));
	...
	foreach my $key (keys %{$Common::config{used_prefix}})
	{
		if( not defined($description{$key}) )
		{
			Common::print_keyLog("SpiderChartAxes");
			Util::print_message("I don't have description for $key area. Look at ".Util::red($in_file));
			exit;
		}
	}
	my $show_warning = 0;
	foreach my $prefix (keys %{$Common::config{used_prefix}})
	{
		if( not defined($Common::config{prefix_priority}{$prefix}) )
		{
			Util::print_message("$Util::icons{X} No priority defined for: ".Util::red($prefix));
			$Common::config{prefix_priority}{$prefix} = ++$Common::config{PrefixPriorityCount};
			$show_warning = 1;
		}
	}
	if( $show_warning == 1 )
	{
		my $DisciplineConfigFile = Common::GetTemplate("discipline-config");
		Util::print_message("$Util::icons{X} See files:\n".Util::red($in_file)."\n".Util::red($DisciplineConfigFile));
	}

	my ($output, $list_of_areas) = ("", "");
	my $MissingAreasCount = 0;
	$output .= "\\begin{enumerate}\n";
	foreach my $key (sort {$Common::config{prefix_priority}{$a} <=> $Common::config{prefix_priority}{$b}}     
	                keys %{$Common::config{used_prefix}})
 	{
...
sub process_critical_path_for_one_course($$$$)
{
	...
	#Util::print_message($tab.Util::yellow("$codcour")."(".Common::format_semester_label_in_plain_text($Common::course_info{$codcour}{semester}, $lang).")->");
...
sub generate_curricula_in_dot_internal($$$$$)
{
	...
	for(my $semester = $Common::config{SemMin}; $semester <= $Common::config{SemMax} ; $semester++)
	{
			#print "$semester ...  ";
			$output_txt .= "$sep";
			$output_txt .= Common::format_semester_label_in_dot($semester, $lang);
			$sep = "->";
	}
	$output_txt .= ";\n";
	my $rank_text = "";
	...
	for(my $semester = $Common::config{SemMin}; $semester <= $Common::config{SemMax} ; $semester++)
	{
		...
		my $sem_text = "";
		my $sem_rank = "";
		my $sem_label = Common::format_semester_label_in_dot($semester, $lang);
	...
	for(my $semester = $Common::config{SemMin}; $semester <= $Common::config{SemMax} ; $semester++)
	{
		...
		foreach my $codcour ( @{$Common::courses_by_semester{$semester}} )
		{
			...
			{
				if( $req =~ m/$Common::config{institution}=(.*)/ )
				{
	...
	my $highlighted_text = Util::highlight_filename($output_dot_file);
	Util::print_message("$Util::icons{ok} ($highlighted_text, <<course_tpl>>, $lang) OK!");
}
...
sub generate_curricula_in_dot($$)
{
	my ($size, $lang) = (@_);
	Util::check_point("detect_critical_path");
	my $output_dot_file = Common::ExpandTags(Common::GetTemplate("out-$size-graph-curricula-dot-file"), $lang);
	my $highlighted_text= Util::highlight("$output_dot_file", Common::GetTemplate("out-$size-graph-curricula-dot-only-file"), \&Util::yellow);
	my $course_tpl 	= Util::read_file(Common::read_dot_template($size, $lang));
	Util::print_message(Util::yellow("generate_curricula_in_dot"));
...
sub generate_map_of_courses_by_outcome($$)
{
	my ($size, $lang) = (@_);
	Util::check_point("detect_critical_path");
	$Common::config{"dots_to_be_generated"}	.= "\n";
	my $version	= $Common::config{OutcomesVersion};
	...
	{	my $courses_counter = keys %{ $Common::config{course_by_outcome}{$outcome} };
		if($courses_counter > 0)
		{
			...
			$Common::config{"dots_to_be_generated"}	.= "echo \"👍 Generating ".Util::highlight_filename("$OutputFigsDir/$filename.svg")." ...\";\n";
			#$Common::config{"dots_to_be_generated"}.= "dot -Tps  $output_dot_file -o $OutputFigsDir/$filename.ps; \n";
			$Common::config{"dots_to_be_generated"}	.= "dot -Tsvg $output_dot_file -o $OutputFigsDir/$filename.svg; \n\n";
		}
	}
...
{
    my ($lang) = (@_);
	my $poster_txt = Util::read_file(Common::GetTemplate("in-poster-file"));
...
# 	$poster_txt =~ s/<DEF_WIDTH>/$Common::config{def_width}$Common::config{logowidth_units}/g;
	# We have to process more poster's content here
        # ..........
        # ..........
        # ..........
    $poster_txt = Common::translate($poster_txt, $lang);
	$poster_txt = Common::ExpandTags($poster_txt, $lang);
	my $OutPosterFile = Common::ExpandTags(Common::GetTemplate("out-poster-file"), $lang);
	Util::write_file($OutPosterFile, $poster_txt);

	system("cp ".Common::GetTemplate("in-a0poster-sty-file")." ".Common::GetTemplate("OutputTexDir")); ;
	system("cp ".Common::GetTemplate("in-poster-macros-sty-file")." ".Common::GetTemplate("OutputTexDir")); ;

    #my $cwd = getcwd();
    #chdir(Common::GetTemplate("OutputFigsDir"));
    #system("rm Bloom.eps");
    #system("ln -s $cwd/".Common::GetTemplate("InFigsDir")."/Bloom.eps");
	#system("rm Bloom-sequence.eps");
    #system("ln -s $cwd/".Common::GetTemplate("InFigsDir")."/Bloom-sequence.eps");
    #chdir($cwd);
	Util::print_message($Util::icons{ok}." ".Util::yellow("generate_poster($lang)")." OK! ".Util::highlight_filename($OutPosterFile));
}
...
{
	my ($type)      = (@_);
	my $output_file = Common::GetTemplate("out-pie-$type-file");
	my $output_txt  = "";
...
# 	my $fig_file = Common::GetTemplate("OutputFigsDir")."/pie-$type*";
...
sub generate_table_topics_by_course($$$$$$$)
{
					...
					keys %list_of_valid_ku)
	{
		...
		if( (defined($Common::bok{$lang}{$ka}{KU}{$ku}{nhTier1}) and $Common::bok{$lang}{$ka}{KU}{$ku}{nhTier1} > 0) || 
		    (defined($Common::bok{$lang}{$ka}{KU}{$ku}{nhTier2}) and $Common::bok{$lang}{$ka}{KU}{$ku}{nhTier2} > 0) )
...
sub generate_all_topics_by_course($)
{
	...
	my $output_file		= Common::GetTemplate("OutputTexDir")."/$file_name_base";
	...
	#my $big_file 	= Common::GetTemplate("OutputTexDir")."/all-$prefix-by-course";
...
sub generate_list_of_outcomes()
{
	...
	my $output_file	= Common::GetTemplate("out-list-of-outcomes");
...
sub generate_list_of_courses_by_outcome($)
{
	...
	my $output_file = Common::GetExpandedTemplateWithLang("list-of-courses-by-outcome", $lang);
...
sub generate_list_of_courses_by_specific_outcome($)
{
	...
	foreach my $outcome (split(",", $Common::config{outcomes_list}{$version}))
	{
							...
							)
		{
			...
			foreach my $codcour ( sort  { $Common::codcour_list_priority{$a} <=> 
			                              $Common::codcour_list_priority{$b}
										}
			...
			#if($courses_counter > 0)
			#{
				my $specific_outcome = $Common::config{specificoutcome}{$lang}{$outcome}{$number}{label};
				my $outcome_txt      = "--Empty--";
				if( defined($Common::config{specificoutcome}{$lang}{$outcome}{$number}{txt}) )
				{	$outcome_txt = $Common::config{specificoutcome}{$lang}{$outcome}{$number}{txt};
					#Util::print_message("Common::config{specificoutcome}{$lang}{$outcome}{$number}{txt}");					
				}
				else
				{	Util::print_message("Common::config{specificoutcome}{$lang}{$outcome}{$number}{txt} ".Util::red("not defined !"));

				}
				
				
				#print Dumper(\%{$Common::config{specificoutcome}{$lang}{$outcome}});
				#exit;

				$this_specific_outcome_txt .= "\\subsection{$outcome) $outcome_txt}\n";
	...
	my $output_file = Common::GetExpandedTemplateWithLang("list-of-courses-by-specific-outcome", $lang);
...
{
	my ($outcome, $lang) = (@_);
	Util::print_message("generate_table_of_courses_for_one_outcome($outcome, $lang) being generated ...");
	my $OutputCompetencesDir = Common::GetExpandedTemplateWithLang("OutputCompetencesDir", $lang);
	...
	my $this_outcome = "";
	my $first_time   = 1;
	foreach my $number  ( sort  { $Common::config{specificoutcome}{$lang}{$outcome}{$a}{priority} <=> 
	                              $Common::config{specificoutcome}{$lang}{$outcome}{$b}{priority}
								}
						  keys %{$Common::config{course_by_specificoutcome}{$outcome}}
						)
	{
		my ($label, $txt) = ("", "");
		if( defined($Common::config{specificoutcome}{$lang}{$outcome}{$number}{label}) )
		{	$label = $Common::config{specificoutcome}{$lang}{$outcome}{$number}{label};		}
		if( defined($Common::config{specificoutcome}{$lang}{$outcome}{$number}{txt}) )
		{	$txt   = $Common::config{specificoutcome}{$lang}{$outcome}{$number}{txt};		}
...
sub generate_table_of_courses_by_specific_outcome($)
{
	...
	my $output_file = Common::GetExpandedTemplateWithLang("table-of-courses-by-specific-outcome", $lang);
...
# 	my $check_label = "\$\\bullet\$";
	...
	Util::write_file($outfile, $output_text);
	Util::print_message($Util::icons{ok}.Util::highlight_filename($outfile)." ... ok!");
...
sub generate_all_outcomes_by_course($)
{
	...
	my $angle	  		= 90;
	my $outfile 	  	= Common::GetTemplate("OutputTexDir")."/outcomes-by-course-$lang_prefix";
	my $output_txt	  	= "";

	Util::print_message(Util::yellow("generate_all_outcomes_by_course"));
	...
	my $big_file 	= Common::GetExpandedTemplateWithLang("in-all-outcomes-by-course-poster", $lang);
...
sub generate_list_of_courses_by_area($)
{
	...
	foreach my $axe (sort {$Common::config{SpiderChartAxesItems}{$a} <=> $Common::config{SpiderChartAxesItems}{$b}}
...
# 			Util::print_message("Common::course_info{$codcour}{$lang}{course_name}=$course_name");

			$this_topic .= "\t\t\\item ". Common::get_course_link($codcour, $lang)."\n";
# 			$this_topic .= "\t\t\\item \\htmlref{$codcour_label. $course_name}{sec:$codcour_label} ";
# 			$this_topic .= " ($semester";
...
# 			$this_topic .= ")\n";
	...
	my $output_file = Common::GetTemplate("out-list-of-courses-per-area-file");
	Util::write_file($output_file, $output_txt);
...
#  		$output_txt .= "\t\\rput[r]{90}($i,-0.2){$Common::config{dictionary}{all_areas}{$axe}}\n";
		#Util::print_message("Common::config{StdInfo}{$standard}{$axe}{max}=$Common::config{StdInfo}{$standard}{$axe}{max}");
		#Util::print_message("Common::config{StdInfo}{$standard}{max}      =$Common::config{StdInfo}{$standard}{max}");
		my $cociente = 0;
		if( $Common::config{StdInfo}{$standard}{max} > 0 )
		{	$cociente = $Common::config{StdInfo}{$standard}{$axe}{max}/$Common::config{StdInfo}{$standard}{max};	}
		else
		{	Util::print_message("Common::config{StdInfo}{$standard}{max} ".Util::red("MUST NOT be zero ..."));		
			my $SpiderChartInfoDir = GetTemplate("SpiderChartInfoDir");
			Util::print_message("Verify ".Util::red($standard)." at $SpiderChartInfoDir ...");
			exit;		
		}
		my ($x,  $y);
		($x, $y) = ($i, Util::round(10*$cociente));
		$out_tex{max} .= "($x,$y)";
		$cociente = 0;
		if( $Common::config{StdInfo}{$standard}{max} > 0 )
		{	$cociente = $Common::config{StdInfo}{$standard}{$axe}{min}/$Common::config{StdInfo}{$standard}{max}; 	}
		($x, $y) = ($i, Util::round(10* $cociente));
...
sub generate_compatibility_with_standards($)
{
	...
	my $lang_prefix = $Common::config{dictionaries}{$lang}{lang_prefix};
	my $count = 0;
	Util::print_message(Util::yellow("generate_compatibility_with_standards"));
	foreach my $axe (keys %{$Common::config{dictionary}{all_areas}})
	{	if( not defined($Common::config{SpiderChartAxesItems}{$axe}) )
		{	Util::print_message("$Util::icons{X} Not defined axe: ". Util::red($axe));
			$count++;
		}
	}
	if( $count > 0 )
	...
	my $OutputFigsDir = Common::GetTemplate("OutputFigsDir");
	my $output_txt = "";
	Util::print_message(Util::yellow("Standards:")." ".Util::yellow("$Common::config{Standards}"));
	foreach my $standard (split(",", $Common::config{Standards}))
	{
		#Util::print_message("$Util::icons{ok} generate_spider_with_one_standard($standard)");
		#print Dumper(\%{$Common::config{type_of_graph}});
		foreach my $type_of_graph (sort keys %{$Common::config{type_of_graph}})
		{
			my $output_file = "$type_of_graph-$Common::config{area}-with-$standard-$lang_prefix";
			Util::print_message(Util::yellow("Generating $output_file ..."));
...
STANDARD_MESSAGE
	...
	my $output_file = Common::GetExpandedTemplateWithLang("out-comparing-with-standards-file", $lang);
	Util::write_file($output_file, $output_txt);
	Util::print_message($Util::icons{ok}.Util::highlight_filename($output_file)." OK!");
}

sub generate_pie_by_levels()
{
	my $output_file = Common::GetTemplate("out-pie-by-levels-file");
	my $output_txt = "";
...
sub read_equivalence_data($$)
{
	...
	my $infile      	= Common::GetExpandedTemplateWithLang("InEquivDir", $lang)    ."/$old_curricula.txt";
	Util::print_message("Reading equivalence $old_curricula ...");
	if(not -e $infile )
	{	Util::print_message("File ".Util::yellow("\"$infile\"")." ".Util::red("does not exist ...")." ignoring this equivalence !");
		my $program_info = Common::GetExpandedTemplateWithLang("this-program-info-file", $lang);
		Util::print_message("I suggest to look at: ".Util::yellow($program_info));
		assert(0);
	}
...
sub generate_equivalence_old2new($$)
{
	...
	my $outfile     	= Common::GetTemplate("OutputTexDir")."/$outfile_short.tex";
...
# 			my $this_line 	= $Common::general_info{equivalences}{$old_curricula}{$semester}{$old_course_codcour}{oldfirstcols};
			...
			if($Common::course_info{$codcour}{bgcolor})
			{
				...
				my $semester_label 	= Common::format_semester_label_in_latex($Common::course_info{$codcour}{semester}, $lang);
		...
		foreach my $codcour (@{$Common::courses_by_semester{$semester}})
		{
			...
			{	my %tags = ();
				...
				foreach my $old_course_codcour (keys %{$Common::course_info{$codcour}{equivalences}{$old_curricula}} )
				{
					...
					$tags{OLD_COURSE_SEM}			= Common::format_semester_label_in_latex($old_semester, $lang);
...
{
	my ($lang) = (@_);
	Util::print_message(Util::yellow("process_equivalences"));
	...
	my $output_equivalences_file = Common::GetTemplate("out-equivalences-file");
	Util::write_file($output_equivalences_file, $equivalences_file_txt);
	Util::print_message($Util::icons{ok}.Util::highlight_filename($output_equivalences_file)." OK!");
}
...
sub generate_courses_by_professor($)
{
	...
	{	my $prefix = $Common::config{faculty}{$email}{fields}{prefix};
		...
		Util::print_message("Processing professor: $name ($email) $number_of_courses_assigned courses assigned ...");
		my $courses_by_professor_count =  0;
		my $this_professor_list        = "";
		
		foreach my $codcour (sort  {$Common::codcour_list_priority{$a} <=> $Common::codcour_list_priority{$b}}
							 #keys %{$Common::config{faculty}{$email}{fields}{courses_i_could_teach}}
							 keys %{$Common::config{faculty}{$email}{fields}{courses_assigned}}
							 )
							 
		{
			$this_professor_list .= "\t\\item \\htmlref{$codcour $Common::course_info{$codcour}{$lang}{course_name}}{sec:$codcour}\n";
			$courses_by_professor_count++;
		}
		if($courses_by_professor_count > 0)
		{
			$out_txt .= "\\item \\textbf{$prefix $name}\n";
	...
	#Util::print_message("Common::config{dictionaries}{$lang}{CoursesAssignedByProfessor}=$Common::config{dictionaries}{$lang}{CoursesAProfessorCouldTeach}}");
	$output_tex .= "\\section{$Common::config{dictionaries}{$lang}{CoursesAssignedByProfessor}}\\label{sec:CoursesAProfessorCouldTeach}\n";
	if( $professor_count > 0 )
	{	$output_tex .= "\\begin{itemize}\n";
	    $output_tex .= $out_txt;
		$output_tex .= "\\end{itemize}\n";
	}
	my $out_file = Common::GetExpandedTemplateWithLang("out-courses-by-professor-file", $lang);
	Util::print_message("$Util::icons{ok} ".Util::highlight_filename($out_file));
...
{
	my ($lang) = (@_);
	Util::print_message(Util::yellow("generate_professor_by_course"));
	my $output_txt  = "% Generated by generate_professor_by_course ... Do not touch !!!\n";
	   $output_txt .= "\\section{$Common::config{dictionaries}{$lang}{ProfessorAssignedByCourse}}\\label{sec:ProfessorAssignedByCourse}\n";
		$output_txt .= "\\begin{itemize}\n";
	...
	foreach my $codcour (@Common::codcour_list_sorted)
	{
		...
		{
			if(not $current_semester == 0) # Close the previous one?
			{	$output_txt .= "\t\\end{itemize}\n\n";	}
			$current_semester = $semester;
			$output_txt .= "\t\\item $Common::config{dictionaries}{$lang}{semester_ordinal}{$semester} $Common::config{dictionaries}{$lang}{Semester}\n";
			$output_txt .= "\t\\begin{itemize}\n";
		}
		my $course_name 		= Common::prepare_text_for_latex($Common::course_info{$codcour}{$lang}{course_name});
		$output_txt .= "\t\t\\item \\htmlref{\\colorbox{$Common::course_info{$codcour}{bgcolor}}{$codcour}. $course_name}{sec:$codcour}\n";
		# $tags{COURSE_NAME} = "\\htmlref{$Common::course_info{$codcour}{$Common::config{language_without_accents}}{course_name}}{sec:$codcour}";

		my $nprofessors_for_this_course = scalar keys %{ $Common::config{distribution}{$codcour}{list_of_professors}};
		...
		{	$output_txt .= "\t\t\t\\item $Common::config{dictionaries}{$lang}{ThereIsNoProfessorAssignedForThisCourse}\n";	}
		else
		{	$output_txt .= "$professor_list";	}
		$output_txt .= "\t\t\\end{itemize}\n";
		$output_txt .= "\n";
	}
	$output_txt .= "\t\\end{itemize}\n"; # Close last semester
	$output_txt .= "\\end{itemize}";
	my $out_file = Common::GetExpandedTemplateWithLang("out-professor-by-course-file", $lang);
	Util::print_message("$Util::icons{ok} ".Util::highlight_filename($out_file)." OK");
...
sub generate_main()
{
	...
	Util::print_message("$Util::icons{ok} Generating main ...");
	...
	{
		my $dir = Common::GetExpandedTemplateWithLang($item, $lang);
		push(@MainsDirsExpanded, $dir);
		my $file = "$dir/Main";
		if( -e "$file.tex" )
		{	Util::print_message("$Util::icons{ok} ".Util::highlight_filename("$file.tex "));
			$number_of_mains_detected++;
			$last_active = $nDirs;	
		}
		else
		{	Util::print_message("$Util::icons{X} ".Util::highlight_filename("$file.tex ").Util::red("Doesn't exist!"));	}
		$nDirs++;
	}
	...
	foreach my $item (@MainsDirsExpanded)
	{
		...
		if($i < $last_active)
		{	if( -e "$file.tex" )
			{	$output_tex .= "% (#$i) EXISTS but ignored ...\n% $MainsDirsExpanded[$i]/Main.tex\n";
				$output_tex .= "% \\input{$MainsDirsExpanded[$i]/Main}\n";
			}
		}
		elsif($i == $last_active)
		{	assert( -e "$file.tex" );
			$output_tex .= "% (#$i) Active: $MainsDirsExpanded[$i]/Main.tex\n";
			$output_tex .= "\\input{\\$MainsDirs[$i]/Main}\n";
		}
		if( not -e "$file.tex" )
		{	$output_tex .=	"% (#$i) It doesn't exist ...\n";
			$output_tex .=	"% \\input{$MainsDirsExpanded[$i]/Main}\n";
		}
		$output_tex .=	"\n";
		$i++;
	}
	my $output_main_file = Common::GetTemplate("out-main-tex-file");
	Util::write_file($output_main_file, $output_tex);
	Util::print_message("$Util::icons{ok} ".Util::highlight_filename($output_main_file)." generated ok! ...");
}

# sub generate_tables_for_advance()
# {
# 	$Common::advances_dir = "$Common::html/$Common::area-$Common::config{institution}-advances";
...
# 	my $html_base = Common::GetTemplate("in-web-course-template.html-file");
# 	#print "$html_base !\n";
...
# 			}
# 			#$out_text =~ s/--INSTITUTION--/$Common::config{institution}/g;
