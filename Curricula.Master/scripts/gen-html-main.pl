...
sub replace_syllabus($)
{
	...
	while($text =~ m/\\course\{(.*?)\}\{(.*?)\}\{(.*?)\}/g)
	{
		...
		$syllabus_head .= "\\input{".Common::GetTemplate("OutputPrereqDir")."/$codcour-html}\n";
...
sub replace_environments($)
{
	...
	Util::print_message("$Common::config{institution}: Syllabi processed: $syllabus_count ...");
	...
	Util::print_message("$Common::config{institution}: Justification: $justification_count");
	...
	Util::print_message("$Common::config{institution}: Goals: $goals_count");
	...
	Util::print_message("$Common::config{institution}: Outcomes: $outcomes_count");
	...
	Util::print_message("$Common::config{institution}: Specific Outcomes: $specific_outcomes_count");
	...
	Util::print_message("$Common::config{institution}: Goals: $objetivos_count");
	...
	Util::print_message("$Common::config{institution}: Topics: $topicos_count");

	($text, $units_count) = replace_unit_environments($text, "unit", "", $Common::config{subsection_label});
	Util::print_message("$Common::config{institution}: Units: $units_count");
	...
	Util::print_message("$Common::config{institution}: Bib files: $bib_count");
...
#     $maintxt =~ s/\{\s*/\{/g;
    ...
    #$maintxt =~ s/\\newcommand{\\comment}/\%\\newcommand{\\comment}/g;
    $maintxt =~ s/\\newcommand\{$Common::config{institution}\}\{.*?\}//g;
    my $country_without_accents = Common::GetTemplate("country_without_accents");
    $maintxt =~ s/\\newcommand\{$country_without_accents\}\{.*?\}//g;
    my $country = Common::GetTemplate("country");
    $maintxt =~ s/\\newcommand\{$country\}\{.*?\}//g;
    my $language_without_accents = Common::GetTemplate("language_without_accents");
    $maintxt =~ s/\\newcommand\{$language_without_accents}\{.*?\}//g;
    my $language = Common::GetTemplate("language");
...
sub main()
{
	...
	my $output_file     = Common::GetTemplate("unified-main-file");
	my $main_file       = Common::GetTemplate("curricula-main");
	...
	while(($changes+$macros_changed+$environments_count) > 0)
	{
		...
		Util::print_message("$Common::config{institution}: Environments = $environments_count");
...
#   		$maintxt =~ s/\(\\$learningoutcome\s*?\)/\(\\textbf\{$Common::config{macros}{$learningoutcome}}\)/g;
	...
	my $OutputPdfInstDir = Common::GetTemplate("OutputPdfInstDir");
...
#   $maintxt = replace_outcomes_sequence($maintxt);
	...
	while ($maintxt =~ m/\n\n\n/){	$maintxt =~ s/\n\n\n/\n\n/g;	}
	$maintxt = Common::replace_latex_label_to_latex_standard($maintxt);
