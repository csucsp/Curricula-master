...
my %ISBOKAreas = ("IT"  => "Tecnologia de Información",
                ...
                "TDS" => "Teoría y Desarrollo de Sistemas de Información"
	       );
my $replacements    = "";
...
sub generate_IS_BOK($)
{
	...
	generate_IS_LU       ($LU_unprocessed_file, Common::GetExpandedTemplateWithLang("InTexDir", $lang)."/LU.tex"));;
	gen_IS_BOK      ($bok_in_file, "$OutputTexDir/IS-bok.tex");
	generate IS_BOK_macros($bok_in_file, Common::GetTemplate("InStyDir")."/bok-macros.sty");
	generate_IS_LU_macros ($LU_unprocessed_file, Common::GetTemplate("InStyDir")."/LU-macros.sty");

	my $replacements_file = Common::GetTemplate("in-replacements-file");
